...
const ArtsAndcrafts = () => {
 ...
 const dispatch=useDispatch();
  ...
  if (loading) {
    return <Lottie animationData={LoadingImage}  style={{width:200,height:100}}/>;
  }
...
const Add_product=(item)=>{
  ...
  return (
    <>
      <h1 style={{ textAlign:'center' }}>Arts And Crafts </h1>
      <div>
        <div className="latest-product-trend">
          <div className="product-container">
            {otherData.filter(item => item.category === 'Art'   || item.category==='cultural' ).map((product, index) => (
              <Card className='card-new-arrival' style={{ width: '18rem' }}  key={index}>
                ...
                <Card.Body>
                  ...
                  <Button variant="info" style={{ color: 'white', fontWeight: 'bold' }} onClick={()=>Add_product(product)}>Order Now</Button>
                </Card.Body>
              </Card>
            ))}
          </div>
